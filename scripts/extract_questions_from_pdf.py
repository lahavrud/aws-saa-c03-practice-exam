...
def extract_text_from_pdf(pdf_path):
    ...
    if PDF_LIB == "PyPDF2":
        ...
        raise ImportError(
            "No PDF library found. Please install one: pip install PyPDF2 pdfplumber pymupdf"
        )
                ...
                if line.startswith("["):
                    ...
                    while i < len(lines):
                        ...
                        if (
                            ...
                            or re.match(r"Q\d+:", next_line, re.IGNORECASE)
                        ):
                            break
    ...
    try:
        ...
        for i, q in enumerate(questions, 1):
            ...
            for idx, opt in enumerate(options):
                opt_text = opt.get(
                    "text", opt.get("answer", opt.get("label", str(opt)))
                )
                is_correct = opt.get("correct", opt.get("isCorrect", False))

                option_list.append({"id": idx, "text": opt_text, "correct": is_correct})
...
def extract_questions_from_wpProQuiz(html_path):
    ...
    try:
        ...
        if wpProQuiz_content:
            ...
            tables = wpProQuiz_content.find_all(
                "table", class_=re.compile(r"wp.*table|list.*table", re.I)
            )
            for table in tables:
                        ...
                        if category_th:
                            ...
                            if category_match:
                                ...
                                if "Cost-Optimized" in current_category:
                                    ...
                                    "High-Performing" in current_category
                                    or "Performance" in current_category
                                ):
                                    current_category = (
                    ...
                    if ths and ths[0].get_text(strip=True).isdigit():
                        ...
                        if question_th:
                            ...
                            question_text = re.sub(
                                r"\(view\)\s*$", "", question_text, flags=re.IGNORECASE
                            ).strip()
                            ...
                            if i + 1 < len(rows):
                                ...
                                if (
                                    detail_row.get("style", "").find("display: none")
                                    != -1
                                    or "display:none"
                                    in detail_row.get("style", "").lower()
                                ):
                                    detail_th = detail_row.find("th")
                                    if detail_th:
                                        # Find options list
                                        options_list = detail_th.find(
                                            "ul",
                                            class_=re.compile(
                                                r"wpProQuiz.*questionList", re.I
                                            ),
                                        )
                                        if options_list:
                                            for idx, li in enumerate(
                                                options_list.find_all("li")
                                            ):
                                                label = li.find("label")
                                                if label:
                                                    option_text = label.get_text(
                                                        strip=True
                                                    )
                                                    if option_text:
                                        ...
                                        # Find explanation
                                        explanation_div = detail_th.find(
                                            "div",
                                            class_=re.compile(
                                                r"wpProQuiz.*response", re.I
                                            ),
                                        )
                                        if explanation_div:
                            ...
                            if question_text and options:
                                domain = (
                                    current_category
                                    if current_category
                                    else determine_domain(question_text)
                                )
                                questions.append(
                                    {
                                        "id": question_num,
                                        ...
                                        "explanation": explanation
                                        if explanation
                                        else f"The correct answer{'s are' if len(correct_answers) > 1 else ' is'} the option{'s' if len(correct_answers) > 1 else ''} marked as correct.",
                                        "domain": domain,
                                    }
                                )
            ...
            if not questions:
                question_divs = wpProQuiz_content.find_all(
                    "div", class_=re.compile(r"question|statistic", re.I)
                )
                for div in question_divs:
                    question_text_elem = div.find(
                        class_=re.compile(r"question.*text|wpProQuiz.*question", re.I)
                    )
                    if question_text_elem:
                        question_text = question_text_elem.get_text(strip=True)
                        if (
                            question_text
                            and len(question_text) > 20
                            and "?" in question_text
                        ):
                            # Look for answer list
                            answer_list = div.find(
                                "ul",
                                class_=re.compile(r"question.*list|answer.*list", re.I),
                            )
                            if answer_list:
                                options = []
                                correct_answers = []
                                for idx, li in enumerate(answer_list.find_all("li")):
                                    ...
                                    if option_text:
                                        is_correct = "correct" in li.get(
                                            "class", []
                                        ) or "answerCorrect" in li.get("class", [])
                                        options.append(
                                            {
                                                "id": idx,
                                                "text": option_text,
                                ...
                                # Extract explanation from wpProQuiz_response div
                                explanation = ""
                                if i + 1 < len(rows):
                                    next_row = rows[i + 1]
                                    response_div = next_row.find(
                                        "div", class_="wpProQuiz_response"
                                    )
                                    if response_div:
                                        # Get text and clean it up
                                        explanation_text = response_div.get_text(
                                            separator="\n", strip=True
                                        )
                                ...
                                if options:
                                    # Use category if available, otherwise determine from text
                                    domain = (
                                        current_category
                                        if current_category
                                        else determine_domain(question_text)
    ...
    try:
        ...
        with open(html_path, "r", encoding="utf-8") as file:
            ...
            if wpProQuiz_questions:
                print(
                    f"✓ Found {len(wpProQuiz_questions)} questions in wpProQuiz format"
                )
            ...
            for script in scripts:
                if script.string:
                    ...
                    try:
                        ...
                        if (
                            "question" in script_text.lower()
                            or "quiz" in script_text.lower()
                        ):
                            # Try to extract JSON
            ...
            if len(text) < 5000 or not re.search(r"[A-Z][^.!?]{30,}\?", text):
                print(
                    f"⚠️  Warning: HTML file appears to be a course listing page, not question content."
                )
                print(
                    f"   Questions may be loaded dynamically. Trying to extract any visible text..."
                )
                print(
                    f"   Tip: If you have a saved quiz page with wpProQuiz_user_content populated, place it in the questions directory."
                )
...
"""
    ...
    sorted_tests = sorted(
        all_tests.items(), key=lambda x: int(x[0].replace("test", ""))
    )
    ...
    for test_key, questions in sorted_tests:
        ...
        for q in questions:
            ...
            js_content += (
                f"            domain: {json.dumps(q['domain'], ensure_ascii=False)},\n"
            )
            js_content += "        },\n"
...
def main():
    ...
    questions_dirs = [
        ...
        os.path.join(project_root, "dojo tests"),
    ]
    ...
    # Check for PDF library (only needed if PDF files are present)
    has_pdf_files = any(f[0] == "pdf" for f in all_files)

    if has_pdf_files and PDF_LIB is None:
    ...
    if PDF_LIB and has_pdf_files:
        print(f"Using PDF library: {PDF_LIB}")
    elif has_pdf_files:
        print("No PDF library needed (only HTML/JSON files found)")
    ...
    if all_tests:
        ...
        for test_key, questions in sorted(
            all_tests.items(), key=lambda x: int(x[0].replace("test", ""))
        ):
